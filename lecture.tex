...
\begin{document}
  ...
  \begin{example} \end{example}
    \begin{enumerate} 
      ...
      %(тривиальное или несобственное подпространство) (остальные называются собственными)
    \end{enumerate}
...
\end{definitionnum} 
  ...
  \end{definition} 
  \begin{definition}
    Рангом матрицы $A$ размера $m\times n$ называется ранг системы её строк. $$\includegraphics[width=4cm]{image/lecture-3.png}$$
  \end{definition} 
  \begin{definition}
  ...
  \begin{itemize}
    ...
    Пусть $V, W, U$ - векторные пространства над $\R$
  ...
  \begin{definition}
    Матрица $T$ , полученная из единичной матрицы $E$, с помощью одного элементарного преобразования над строками или столбцами, называется элементарной матрицей. \\
  ...
  \begin{example1} 
    ...
    \end{matrix}
    \Longrightarrow M_{21} = \begin{vmatrix}
      2 && 3 \\ 8 && 9
    \end{vmatrix} = -6$$ 
  \end{example1}
  ...
  \begin{theorem}
    ...
    Пусть $A = (a_{ij})$ - квадратная невырожденная матрица порядка $n$, тогда $\exists$ обратная матрица к $A \ $ и её элементы могут быть найдены по формуле: 
  ...
  \begin{proof} \tab
    ...
    $\Longrightarrow $ $f(a^{-1})$ является обратным к $f(a)$, так как по определению обратным к $f(a)$ является $f(a)^{-1}$ и такой элемент единственен $\Longrightarrow $ $f(a^{-1}) = f(a)^{-1}$\\
...
\end{remark} 
\begin{definition}
  Циклы $(i_1,...,i_k)$ и $(j_1,...,j_t)$ называются независимыми, если: $$\{i_1,...,i_k\} \cap \{j_1,...,j_t\} = \varnothing $$  
\end{definition} 
\begin{example1}
  $(1,2,3), \ (4,5)$ 
\end{example1}
...
\begin{proof} \tab
  ...
  \begin{proof}
    ...
    при $ k = \{0,1,...,n-1\}$ - $w$ принимает все различные значения.  
  \end{proof} 
  \begin{example1}
    $z=1,\ n=3, \ \sqrt[3]{1}$ 
  \end{example1}
  ...
  Пусть $F$ - поле 
  \begin{definition}
    Алгеброй над полем $F$ называется множество $A$ с операциями сложения, умножения и умножения на элементы поля, удовлетворяющее следующим аксиомам:
    \begin{enumerate}
      \item $(A, +, \cdot)$ - кольцо
  ...
  \begin{example}\tab
    \begin{enumerate}
      \item $M_n(F) -$ алгебра матриц с коэффициентами из $F$ (это НЕкоммутативная, ассоциативная с единицей алгебра над $F$)
  ...
  \begin{example1}
    ...
    $$\begin{tabular}{c|c|c|c|c}
        ...
        k & k & j & i & -1
      \end{tabular}$$
      $\Longrightarrow $ ассоциативная, некоммутативная алгебра, в которой каждый не нулевой элемент обратим (тело).  
  \end{example1}
  \begin{definition}
    Подмножество $B$ алгебры $A$ называется подалгеброй $A$, если $B$ - подпространство $A$, как кольца, и подпространство $A$, как пространства.      
  \end{definition} 
  \begin{subtheorem}
  ...
  $F$ - поле
  \begin{definition}
    Бесконечная последовательность $(a_0,a_1, a_2,...)$, где $a_i \in F$, называется финитной, если только конечное число $a_i$ отлично от нуля. 
  ...
  \begin{proof}
    ...
    \end{multline*} 
     $$1-tC(n-s)>0 \Longleftrightarrow t<\frac{1}{C(n-s)}$$
  ...
  \begin{definition}
    ...
    Он называется старшим и обозначается: $LT(f)$  
  \end{definition}
  \begin{example1}
  ...
  \begin{definition}
    Многочлен $f \in F[x_1,...,x_n]$ называется симметрическим, если:
  ...
  \section*{Заключение}
  В заключение выражаю благодарность главным редакторам конспекта: \\Егору Соколову (108 группа), Кириллу Яковлеву (108 группа), Ярославу Светлакову (108 группа), Егору Цыбулину (108 группа), Zerx (112 группа) а также Марии Свириной (111 группа) за рукописные конспекты.\\
  С вами был Вячеславик $\heartsuit$ 
  \begin{center}
    \includegraphics[width=3cm]{image/kotik.jpg}
  \end{center}
