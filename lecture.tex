...
\end{definition} 
  ...
  \begin{enumerate}
    ...
    $$[\widetilde{D}]_{ij} = [(AB)C]_{ij} = \sum \limits_{t=1}^n[AB]_{it}[C]_{tj} = \sum \limits_{s=1}^n (\sum \limits_{s=1}^k[A]_{is} \cdot [B]_{st})[C]_{tj} = $$
    $$ = \sum \limits_{s=1}^n \sum \limits_{s=1}^k ([A]_{is} \cdot [B]_{st}) \cdot [C]_{tj}$$  
    По свойствам операций над $\R$, результаты преобразований равны.
    \end{proof} 
    \item $A(B+C) = AB + BC$
    \item $(B + C)A = BA + CA$  
    \item $\lambda(BC) = (\lambda A) B = A(\lambda B); \tab[0.3cm]\forall \lambda \in \R$ 
    \item $\forall A \in M_{m \times n}(\R)$, единичная матрица $E \in M_{m \times m}(\R)$ : 
    $EA = A$  
  ...
  \end{example1}
  \begin{definition}
    Перестановка $(1,2,...,n)$ назавается тривиальной.
  \end{definition} 
  \begin{definition}
    Говорят, что пара элементов $k_i \text{ и } k_j$ образуют инверсию, если $i<j$, то $k_i>k_j$.
  \end{definition} 
  \begin{definition}
    Перестановка назавается четной (нечетной), если число инверсий в ней четное (нечетное).\\
    Знак переставки $sgn(k_1,...,k_n) = (-1)^s$, где $s$ - число инверсий в перестановке. 
  \end{definition} 
  \begin{definition}
    Перемена двух элементов в перестановке назавается транспозицией этих элементов.
  \end{definition} 
  \begin{subtheorem}
  ...
  \end{proof} 
  \begin{consequense}
    Пусть $n>1$. Тогда число четных перестановок из $n$ элементов равно числе нечетных. 
  \end{consequense} 
  \begin{proof}
