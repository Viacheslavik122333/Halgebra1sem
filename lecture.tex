...
\begin{proof} \tab
  ...
  \end{proof}
  \begin{remark}
    Теория решения СЛУ (метод Гаусса, праивло Крамера, ...), теория определителей, утверждения о векторных пространсвах (в частности о матрицах), которые мы рассматривали раннее, переносятся с $\R$ на произвольные поля. \\
    \textbf{Исключение}  - поле характеристики 2: в определение кососимметричной и полилинейной функции надо трбовать, что при 2 совпадающих аргументов \\ $f(...,v,...,v,...)=0$. Отсюда получаем, что $f(...,v,...,w,...) = -f(...,w,...,v,...)$ (при $charP = 2$ \ \ $1 \neq-1$)  
  \end{remark}
  \subsection{Поле комплексных чисел}
  \textbf{xd} 
  \begin{definition}
    Поле комплексных чисел $\mathbb{C} $ - это:
    \begin{enumerate}
  ...
  \begin{remark}
    ...
    $i$ - мнимая единица. \\
    На декартовой плоскоси: РИСУНОК (Надо изучить векторную графику)\\
  ...
  \begin{example1}
    ...
    $\mathbb{H}$ - 4-х - мерное векторное пространство над $\R$ с базисом $\{1, i, j, k\}$ и таблицей умнодения: МАТРИЦА
  \end{example1}
  \begin{definition}
