...
\begin{document}
  ...
  \end{lemma} 
  \begin{proof}
    Пусть $w_1 = c_{11}v_1+\cdots+c_{1k}v_k, w_2 = c_{21}v_1+\cdots+c_{2k}v_k$,...,$w_m = c_{m1}v_1+\cdots+c_{mk}v_k$, где $c_{ij} \in \R$ \\
    Докажем, что $\exists$ нетривиальное ЛК $w_1,...,w_m=0$ \\
    Для произвольных $\lambda_1,...,\lambda_m$ рассмотрим выражение: $$\lambda_1w_1 +\cdots+\lambda_mw_m = \lambda_1(c_{11}v_1+\cdots+c_{1k}v_k) + \cdots + \lambda_m(c_{m1}v_1+\cdots+c_{mk}v_k) = (\lambda_1c_{11}+\cdots+\lambda_mc_{m1})v_1+\cdots+(\lambda_1c_{1k}+\cdots+\lambda_mc_{mk})v_k$$   
  ...
  \end{proof} 
  \subsubsection{Линейная оболочка множества S}
  $V$ - векторное простанство над $\R$ \\
  $S \subseteq V, S \neq \varnothing $  \\
  \begin{subtheorem}
    Множество всех ЛК $\lambda_1s_1+\cdots+\lambda_ks_k, \lambda_i \in \R, s_i \in S$ образует векторное подпространство в пространстве $V$. 
  \end{subtheorem}  
  \begin{proof}
    Д/з. 
  \end{proof}
  \begin{definition}
    Это векторное подпространство называется линейная оболочка множества $S$, обозначается $\langle S \rangle$.
  \end{definition} 
  \begin{example} $\tab$ 
    \begin{enumerate}
      \item $\R^3, S = \{(1,0,0),(0,1,0)\};$ \tab[0.5cm]
      $\langle S \rangle = \{(\lambda,\mu,0)|\lambda,\mu \in \R\}$ 
      \item $V^3, S =\{i,j,i+j\}$ РИСУНОК
    \end{enumerate}
  \end{example}
  \begin{definition}
...
\begin{lemma} (Переформулировка ОЛЛЗ)
  Пусть векторного пространства $V$ пораждается $k$ векторами. Тогда любые $m>k$ векторов из $V$ - ЛЗ.
\end{lemma} 
\subsection{Базис}
...
\begin{definition}
  ...
  \begin{enumerate}
    \item $\{e_1,...,e_n\}$ - ЛНЗ
    \item $V = \langle e_1,...,e_n \rangle$, т.е. $\forall x \in V \tab[0.1cm] \exists x_1,...,x_n \in \R: x = x_1e_1+\cdots + x_ne_n$  
  \end{enumerate}
...
\end{definition} 
  ...
  \begin{proof}
    ...
    Если $S$ - ЛЗ, то по критерию о ЛЗ один из векторов $s1$ множества $S$ линейно выражается через остальные \\
    Тогда $s1$ = $S\setminus\{s1\}$ - конечное пораждащее множество. ч.т.д. \\
    Т.к. $S$ - конечное, это процесс прервется и мы получим ЛНЗ порожденную систему.
  \end{proof} 
  \begin{theorem}
    В любом базисе конечномерном векторном пространстве $V$ над $\R$ ожно и тоже число векторов.
  \end{theorem} 
  \begin{proof}
  ...
  \end{remark} 
   Пусть $V$- векторное пространство над $\R$, $dimV=n, S\subseteq V$ Любые $m>n$ векторов в $S$- ЛЗ. (из ОЛЛЗ) \\
   $\Longrightarrow $ в $S \ \exists $ максимальный ЛНЗ подсистему (т.е. ничего нельзя добавить к этой подсистеме без нарушения ЛНЗ) 
